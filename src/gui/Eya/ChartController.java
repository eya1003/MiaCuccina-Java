 ...
 */
  ...
  public class ChartController implements Initializable {
    ...
    @FXML
    private AnchorPane chartNode1;
    @FXML
    private AnchorPane chartNode2;
...
// String requete1 = "SELECT * from Reclamation"; //MAJUSCULE NON OBLIGATOIRE 
 String requete = "SELECT date,COUNT(id) as nbr FROM `avis` group by date";
// String requete = "SELECT r.ref_bonplan,COUNT(r.id) as nbr FROM Reclamation r,bon_plan b where r.ref_bonplan=b.id group by r.ref_bonplan";
            //String requete = "SELECT date,COUNT(avis.id) as nbr FROM avis left join avis on avis.date=avis.id group by avis.date";

            Statement pst = MyDB.getInstance().getConnexion().prepareStatement(requete); // import java.sql.Statement
            rs = pst.executeQuery(requete);
            while (rs.next()) {

                if (rs.getObject(1) == null) {
                    System.out.println(rs.getString(1));
                    d = new PieChart.Data("Avis ", rs.getInt(2));
                } else {
   ...
   public ObservableList buildDataCompte() {
    // public  ObservableList<PieChart.Data> buildData() {
        ...
        try {
// String requete1 = "SELECT * from Reclamation"; //MAJUSCULE NON OBLIGATOIRE 
 String requete = "SELECT type,COUNT(idU) as nbr FROM `avis` group by type";
// String requete = "SELECT r.ref_bonplan,COUNT(r.id) as nbr FROM Reclamation r,bon_plan b where r.ref_bonplan=b.id group by r.ref_bonplan";
           // String requete = "SELECT admin.username,COUNT(admin.idU) as nbr FROM admin left join admin on Reclamation.ref_compte=fosuser.id group by Reclamation.ref_compte";
          ...
          Statement pst = MyDB.getInstance().getConnexion().prepareStatement(requete); // import java.sql.Statement
            ...
            while (rs.next()) {
                ...
                if (rs.getObject(1) == null) {
                    System.out.println(rs.getString(1));
                    d = new PieChart.Data("Type ", rs.getInt(2));
                } else {
                    d = new PieChart.Data(rs.getString(1), rs.getInt(2));
                }
//                System.out.println(rs.getString(1));
//                System.out.println(rs.getInt(2));
     ...
     public XYChart.Series buildDataLineChart() {
        ...
        XYChart.Series d;
        try {
            String requete = "SELECT avis.date,COUNT(avis.id) as nbr FROM avis group by DAYNAME(avis.date)";
    ...
    private void detailleBonPlan() {
        ...
        chart.setTitle("Statistique Avis");
        ((Group) scene.getRoot()).getChildren().add(chart);
        stage.setScene(scene);
